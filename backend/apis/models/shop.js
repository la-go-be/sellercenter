module.exports = function (sequelize, DataTypes) {
    ...
    function getStore() {
        return Shop.findAll({
            attributes: [['id', 'shopID'], ['user_account_id', 'userAccountID'], ['store_name', 'storeName']],
            raw: true
        }).then(function(shop) {
            return shop;
        });
    ...
    function getByUserAccountID2(userAccountID) {
        return sequelize.query(`
            SELECT
                shop.store_name,
                CASE
            ...
            LEFT JOIN bank ON shop.bank_id = bank.id
            LEFT JOIN \`company.prefix\` cprefix ON shop.store_company_register_company_prefix = cprefix.id
            WHERE shop.user_account_id=:userAccountID
        `, {
